...
export function useLocalData() {
  ...
  const setupListeners = useCallback(() => {
    ...
    // Config: images - ESCUCHA EN TIEMPO REAL DESDE FIREBASE
    unsubs.current.push(onSnapshot(doc(db, 'config', 'images'), (snap) => {
      if (snap.exists()) {
        const d = snap.data() as ImagesConfig;
        console.log('🖼️ Imágenes actualizadas desde Firebase:', { 
          logoUrl: d.logoUrl?.substring(0, 50) + '...', 
          heroImage: d.heroImage?.substring(0, 50) + '...' 
        });
        setImages(d);
        persist(KEYS.images, d);
      } else {
        console.log('⚠️ No hay documento de imágenes en Firebase, usando defaults');
      }
    }, (e) => console.error('❌ Error escuchando imágenes:', e.message)));
    ...
    unsubs.current.push(onSnapshot(collection(db, 'menuItems'), (snap) => {
      if (!snap.empty) {
        ...
        console.log('✅ Productos cargados desde Firebase:', d.length);
      }
    ...
    unsubs.current.push(onSnapshot(collection(db, 'categories'), (snap) => {
      if (!snap.empty) {
        ...
        console.log('✅ Categorías cargadas desde Firebase:', d.length);
      }
  ...
  useEffect(() => {
    if (products.length > 0 && categories.length > 0) {
      console.log('🎉 Datos completos cargados - UI lista para mostrar');
  ...
  useEffect(() => {
    // Activar listeners INMEDIATAMENTE (sin esperar nada)
    setupListeners();
    console.log('⚡ Listeners activados - UI instantánea');

    // Seed de Firebase en segundo plano (sin bloquear)
    const seedFirebase = async () => {
      try {
        console.log('🔥 Firebase: verificando datos en segundo plano...');
        
        const snap = await getDoc(doc(db, 'config', 'business'));
        
        if (!snap.exists() && !didSeed.current) {
          didSeed.current = true;
          console.log('🌱 Sembrando datos en Firebase (background)...');

          // Sembrar en paralelo para mayor velocidad
          await Promise.all([
            setDoc(doc(db, 'config', 'business'), { ...defaultConfig }),
            setDoc(doc(db, 'config', 'images'), { ...defaultImages }),
            setDoc(doc(db, 'config', 'about'), { ...defaultAbout }),
            setDoc(doc(db, 'config', 'social'), { ...defaultSocial }),
          ]);
          console.log('✅ Configuración sembrada');

          // Sembrar categorías en paralelo
          const categoryPromises = defaultCategories.map(c => 
            setDoc(doc(db, 'categories', c.id), { name: c.name, icon: c.icon })
          );
          await Promise.all(categoryPromises);
          console.log('✅ Categorías sembradas');

          // Sembrar productos en paralelo (lotes de 10)
          const productPromises = defaultProducts.map(p =>
            setDoc(doc(db, 'menuItems', p.id), {
        ...
        console.warn('⚠️ Firebase seed en background:', error.message);
        // La app YA está funcionando con localStorage
      }
    };

    // Ejecutar seed en segundo plano (no bloquea la UI)
    seedFirebase();
  ...
  return {
    products, categories, config, images, about, social, reservations, loading, isDataLoaded,
